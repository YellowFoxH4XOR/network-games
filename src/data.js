...
//
// The bank is authored in per-topic arrays so the Spin Wheel (stall 1) can
// serve a question from whatever topic the wheel lands on; QUIZ_QUESTIONS
// below flattens them in the same order as before for the regular quiz.
...
// Flat bank in the original authoring order — the regular quiz's source.
export const QUIZ_QUESTIONS = [
  ...SWITCH_QS, ...ROUTER_QS, ...LOAD_BALANCER_QS, ...FIREWALL_QS, ...WIFI_QS,
  ...SERVER_QS, ...IP_DNS_QS, ...PROTOCOL_QS, ...SECURITY_QS, ...PERFORMANCE_QS,
];

// The Spin Wheel's 8 segments (stall 1). Labels are what the wheel shows;
// each spin serves one random question from the landed topic's pool.
export const SPIN_TOPICS = [
  { label: 'Switches',       questions: SWITCH_QS },
  { label: 'Routers',        questions: ROUTER_QS },
  { label: 'Load Balancers', questions: LOAD_BALANCER_QS },
  { label: 'Firewalls',      questions: FIREWALL_QS },
  { label: 'Wi-Fi',          questions: WIFI_QS },
  { label: 'Servers & LAN',  questions: SERVER_QS },
  { label: 'IP & DNS',       questions: IP_DNS_QS },
  { label: 'Protocols',      questions: PROTOCOL_QS },
];
...
// One spin's question: a random question from the landed topic, options
// shuffled. `exclude` (question texts already asked this run) keeps a repeat
// topic from repeating a question until its pool is exhausted.
export function spinQuestion(topicIndex, exclude = []) {
  const pool = SPIN_TOPICS[topicIndex].questions;
  const fresh = pool.filter(q => !exclude.includes(q.question));
  const pick = fresh.length ? fresh : pool;
  return shuffleOptions(pick[Math.floor(Math.random() * pick.length)]);
}
